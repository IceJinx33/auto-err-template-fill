...
def transform(point1, point2, matching, inner = False):
    ...
    point1_string, point2_string = (None if point1 is None else "/".join([str(p) for p in point1])), (None if point2 is None else "/".join([str(p) for p in point2]))
    result = Result()
    if (point1 is not None and len(point1) == 1) or (point2 is not None and len(point2) == 1):
        ...
        if point1 is None: 
            result.log += "Add item: "+point2_string
            result.stats[point2[1]]["r_den"] += 1
            result.stats["total"]["r_den"] += 1
            result.errors["Missing_Role_Filler"] += [point2_string]
            result.error += 1
        elif point2 is None: 
            result.log += "Remove item: "+point1_string
            result.stats[point1[1]]["p_den"] += 1
            result.stats["total"]["p_den"] += 1
            result.errors["Spurious_Role_Filler"] += [point1_string]
            result.error += 1
        else:
            if not inner:
                result.stats[point1[1]]["p_den"] += 1
            ...
            #if not inner: result.log += point1_string+" <-> "+point2_string
            if False: #([point1[0]],[point2[0]]) not in matching:
                pass
                #print(point1_string+" -> (change template)")
                #result = Result.combine(result, transform(matching))
            elif point1[1] != point2[1]:
                result.log += ("\n" if inner else "")+point1_string+" => (change role)"
                result.errors["Incorrect_Role"] += [point1_string]
                result.error += 1
                ...
                if isinstance(point1[2], str) and isinstance(point2[2], str):
                    ...
                    result.error += 1
                    result.errors["Incorrect_Incident_Type"] += [point1.string]
                    result = Result.combine(result, transform(point1[:2]+[point2[2]]+point1[3:], point2, matching, inner = True), close = True)
                elif type(point1[2]) is tuple and type(point2[2]) is tuple:
                    score = span_scorer(point1[2], point2[2])
                    if score < 1:
                ...
                if inner: result.log += "\n"+point2_string+' =| (done)'
                    ...
                    result.log += point1_string+" = "+point2_string+' =| (done)'
...
class Solution:
    ...
    def compute(self):
        ...
        for pair in self.matching:
            result = Result.combine(result, transform(*pair, self.matching))
        return result
...
def generate_solutions(predicted_points, gold_points):
    ...
    for template_matching in generate_matchings(len(predicted_templates), len(gold_templates)):
        for point_matching in generate_matchings(len(predicted_singles), len(gold_singles)):
            ...
            for role_name, role_data in gold_temp.items():
                ...
                for entity in role_data:
                    for mention in entity:                        
                        mention_tokens = normalize_string(mention).split()
                        span = mention_tokens_index(doc_tokens, mention_tokens)
                        gold_summary.append(role_text+[span])
